...
router.get("/me", authenticateToken, async (req, res) => {
  try {
    ...
    if (user.isTokenExpired()) {
      return res
        .status(401)
        .json({ message: "Token expired", needsRefresh: true });
    }
...
router.post("/verify", authenticateToken, async (req, res) => {
  try {
    ...
    if (!user.isActive) {
      return res
        .status(403)
        .json({ valid: false, message: "Account is inactive" });
    }
...
router.get("/stats", authenticateToken, async (req, res) => {
  try {
    ...
    const publishedForms = await Form.countDocuments({
      userId,
      isPublished: true,
      isActive: true,
    });
    const draftForms = await Form.countDocuments({
      userId,
      isPublished: false,
    });
...
// @route   POST /api/auth/airtable/pat-login
// @desc    Login using Personal Access Token (bypass OAuth)
// @access  Public
router.post('/airtable/pat-login', async (req, res) => {
  try {
    console.log('🚀 Starting PAT login...');
    console.log('PAT token exists:', !!process.env.AIRTABLE_PERSONAL_ACCESS_TOKEN);
    
    // Test the PAT by getting user info
    ...
    console.log('✅ Full Airtable user data:', JSON.stringify(airtableUser, null, 2));
    
    // Extract user information - Airtable API returns different structure
    const userId = airtableUser.id;
    const userName = airtableUser.name || 'Airtable User';
    const userEmail = airtableUser.email || `${userId}@airtable.local`; // Fallback email if not provided
    
    console.log('📝 Extracted data:', { userId, userName, userEmail });
    ...
    if (!user) {
      ...
      user = new User({
        email: userEmail,
        airtableUserId: userId,
        airtableAccessToken: process.env.AIRTABLE_PERSONAL_ACCESS_TOKEN,
        profile: {
          name: userName,
          email: userEmail
        }
      ...
      user.airtableAccessToken = process.env.AIRTABLE_PERSONAL_ACCESS_TOKEN;
    ...
    if (error.response?.status === 401) {
      res.status(400).json({ 
        error: 'Invalid Personal Access Token. Please check your token in .env file.' 
      });
      ...
      res.status(500).json({ 
        error: 'Failed to authenticate with Personal Access Token: ' + error.message 
      });
    }
  }
});
